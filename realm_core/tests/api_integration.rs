...
use actix_web::{test, web, App, http::header::AUTHORIZATION};
use realm_core::monitor::{ConnectionMetrics, TCP_CONNECTION_METRICS, UDP_ASSOCIATION_METRICS};
use realm_core::api::{list_tcp_connections, get_tcp_connection_stats, list_udp_associations, get_udp_association_stats, Authenticate}; // Import Authenticate
...
const TEST_API_TOKEN: &str = "test-token-for-integration-tests";

fn setup_test_app(token: Option<String>) -> App<impl actix_web::dev::ServiceFactory<
...
>> {
    App::new()
        .wrap(Authenticate::new(token)) // Add authentication middleware
...
async fn test_tcp_stats_endpoints_integration() {
    ...
    env::set_var("API_AUTH_TOKEN", TEST_API_TOKEN);
    let token_for_app = env::var("API_AUTH_TOKEN").ok().filter(|s| !s.is_empty());
    ...
    let srv = test::init_service(setup_test_app(token_for_app)).await;

    // Test unauthorized access
    let req_no_token = test::TestRequest::get().uri("/rules/tcp").to_request();
    let resp_no_token = test::call_service(&srv, req_no_token).await;
    assert_eq!(resp_no_token.status(), actix_web::http::StatusCode::UNAUTHORIZED);

    let req_wrong_token = test::TestRequest::get().uri("/rules/tcp")
        .insert_header((AUTHORIZATION, "Bearer wrongtoken"))
        .to_request();
    let resp_wrong_token = test::call_service(&srv, req_wrong_token).await;
    assert_eq!(resp_wrong_token.status(), actix_web::http::StatusCode::UNAUTHORIZED);

    // Test GET /rules/tcp with correct token
    let req_list = test::TestRequest::get().uri("/rules/tcp")
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
    ...
    // Test GET /rules/tcp/{conn_id}/stats for conn_id1 with correct token
    let req_conn1 = test::TestRequest::get().uri(&format!("/rules/tcp/{}/stats", conn_id1))
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
    let resp_conn1: Value = test::call_and_read_body_json(&srv, req_conn1).await;
    assert_eq!(resp_conn1["tx_bytes"], 1000);

    // Test GET /rules/tcp/{conn_id}/stats for a non-existent ID with correct token
    let non_existent_id = Uuid::new_v4().to_string();
    let req_non_existent = test::TestRequest::get().uri(&format!("/rules/tcp/{}/stats", non_existent_id))
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
...
async fn test_udp_stats_endpoints_integration() {
    ...
    let srv_udp = test::init_service(setup_test_app(token_for_app)).await;

    // Test unauthorized access
    let req_no_token_udp = test::TestRequest::get().uri("/rules/udp").to_request();
    let resp_no_token_udp = test::call_service(&srv_udp, req_no_token_udp).await;
    assert_eq!(resp_no_token_udp.status(), actix_web::http::StatusCode::UNAUTHORIZED);

    let req_wrong_token_udp = test::TestRequest::get().uri("/rules/udp")
        .insert_header((AUTHORIZATION, "Bearer wrongtoken"))
        .to_request();
    let resp_wrong_token_udp = test::call_service(&srv_udp, req_wrong_token_udp).await;
    assert_eq!(resp_wrong_token_udp.status(), actix_web::http::StatusCode::UNAUTHORIZED);

    // Test GET /rules/udp with correct token
    let req_list_udp = test::TestRequest::get().uri("/rules/udp")
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
    ...
    // Test GET /rules/udp/{client_addr}/stats for addr1 with correct token
    let req_addr1 = test::TestRequest::get().uri(&format!("/rules/udp/{}/stats", addr1_str))
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
    let resp_addr1: Value = test::call_and_read_body_json(&srv_udp, req_addr1).await;
    assert_eq!(resp_addr1["tx_bytes"], 500);

    // Test with invalid client_addr format with correct token
    let req_invalid_addr = test::TestRequest::get().uri("/rules/udp/invalid-addr/stats")
        .insert_header((AUTHORIZATION, format!("Bearer {}", TEST_API_TOKEN)))
        .to_request();
